...
from wordcloud import (
    WordCloud,
)  # Wordcloud: a Python library to create a word cloud of common words / phrases
...
class TwitterConnector:
    ...
    def setUpConnection(self):
        ...
        authenticate = tweepy.OAuthHandler(
            self.TwitterAccessKeys["consumerKey"], self.TwitterAccessKeys["consumerSecret"]
        )
        # Set the access token and the access token secret
        authenticate.set_access_token(
            self.TwitterAccessKeys["accessToken"], self.TwitterAccessKeys["accessTokenSecret"]
        )
...
class DataPlotter:
    ...
    def plotWordCloud(self, _df):
        ...
        wordCloud = WordCloud(
            width=3000, height=2000, background_color="white", mask=mask
        ).generate(text)
...
while valid == False:
    ...
    if inputType.lower() == "user":
        ...
        user = str(
            input(
                "Please enter the Twitter handle of the user you wish to receive sentiment analysis about: "
            )
        )
        user.lower()
        try:
    ...
    # This elif statement checks if the user answered with 'keyword'
        ...
        keyword = str(
            input("Please enter the keyword you wish to receive sentiment analysis about: ")
        )
        keyword.lower()
        try:
    ...
    # If the input is not as required, the user is asked again until an appropriate answer is given
        ...
        inputType = str(
            input(
                "Input does not match the options provided. Please try again. Enter 'user' or 'keyword': "
            )
        )
        valid = False
...
if tweets_found == False:
    ...
    plotter.plotPieChart(
        positiveTweetsPercentage, negativeTweetsPercentage, neutralTweetsPercentage
    )
